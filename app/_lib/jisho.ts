...
import type { JishoAPI } from "./jisho.types";
...
function getIntBetweenStrings(pageHtml, startString, endString) {
	const stringBetweenStrings = getStringBetweenStrings(
		pageHtml,
		startString,
		endString,
	);
	if (stringBetweenStrings) {
		return Number.parseInt(stringBetweenStrings, 10);
	}
...
function getYomi(pageHtml, yomiLocatorSymbol) {
	const yomiSection = getStringBetweenStrings(
		pageHtml,
...
function getYomiExamples(pageHtml, yomiLocatorSymbol) {
	...
	const exampleSection = getStringBetweenStrings(
		pageHtml,
		locatorString,
		"</ul>",
	);
	...
	const regexResults = getAllGlobalGroupMatches(exampleSection, regex).map(
		(s) => s.trim(),
	);
...
function getRadical(pageHtml) {
	...
	if (radicalMeaning) {
		const radicalMeaningStartIndex = pageHtml.indexOf(
			radicalMeaningStartString,
		);
...
function getNewspaperFrequencyRank(pageHtml) {
	const frequencySection = getStringBetweenStrings(
		pageHtml,
		'<div class="frequency">',
		"</div>",
	);
...
function normalizeSentenceElement(sentenceElement) {
	...
	const normalizedSentenceHtml = sentenceHtml
		.replace(
			/(?<=<\/li>)\s*([^\s<>]+)\s*(?=<li)/g,
			(m, g1) =>
				`<li class="clearfix"><span class="unlinked">${g1.trim()}</span></li>`,
		)
		.replace(
			/\<\/li\>\s*([^<\s]+)\s*\<\/ul>/g,
			(m, g1) =>
				`</li><li class="clearfix"><span class="unlinked">${g1.trim()}</span></li></ul>`,
		)
		.replace(
			/<ul class="japanese_sentence japanese japanese_gothic clearfix" lang="ja">\s*([^<\s]+)\s*<li/g,
			(m, g1) =>
				`<ul class="japanese_sentence japanese japanese_gothic clearfix" lang="ja"><li class="clearfix"><span class="unlinked">${g1.trim()}</span></li><li`,
		);
...
function getPieces(sentenceElement) {
	const pieceElements =
		normalizeSentenceElement(sentenceElement)("li.clearfix");
	const pieces = [];
...
function getMeaningsOtherFormsAndNotes($) {
	...
	for (
		...
		if (child.hasClass("meaning-tags")) {
			mostRecentWordTypes = child
				.text()
				.split(",")
				.map((s) => s.trim().toLowerCase());
		} else if (mostRecentWordTypes[0] === "other forms") {
			returnValues.otherForms = child
				.text()
				.split("、")
			...
			const meaningAbstract = child
				.find(".meaning-abstract")
				.find("a")
				.remove()
				.end()
				.text();

			const supplemental = child
				.find(".supplemental_info")
				.text()
				.split(",")
				.map((s) => s.trim())
				.filter((s) => s);
			...
			for (
				let sentenceIndex = 0;
				sentenceIndex < sentenceElements.length;
				sentenceIndex += 1
			) {
...
const API: JishoAPI = {
	...
	searchForKanji(kanji) {
		...
		return axios
			.get(uri)
			.then((response) => parseKanjiPageData(response.data, kanji));
	},
	...
	searchForExamples(phrase) {
		...
		return axios
			.get(uri)
			.then((response) => parseExamplePageData(response.data, phrase));
	},
};
